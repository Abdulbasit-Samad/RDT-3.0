...
while True:
    try:
        ...
        if ('0' not in sum) and (req==int(message[2])):
            ...
            print("Received packet", message[0], "with sequence number", message[2])
            print("Sent acknowledgement with sequence number", message[2])
        elif ('0' in sum) or (req!=int(message[2])):
            modifiedMessage = 'AK'+'/r'+'1011111010110100'+'/r'+str(ackseq)
            serverSocket.sendto(modifiedMessage.encode(), clientAddress)
            print("Received packet", message[0], "with sequence number", message[2])
            print("Sent acknowledgement with sequence number", ackseq)
