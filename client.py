...
def rdt_send(data, serverName, serverPort, clientSocket):
    ...
    for i in range(len(packetss)):
        ...
        if random.random() < 0.3:  # 30% chance of packet loss
            print("Packet Loss: Packet", i, "lost")
            continue
        ...
        while True:
            ...
            print("Packet", i, "sent successfully to the server")

            # Wait for acknowledgment
            try:
                clientSocket.settimeout(1)  # Set a timeout for acknowledgment reception
                modifiedMessage, serverAddress = clientSocket.recvfrom(2048)
                clientSocket.settimeout(None)  # Reset the timeout after acknowledgment reception
                ...
                ff = summ(s)
                if '1' in ff or sequence_number == int(modifiedMessage[2]):
                    print("Acknowledgment of packet with sequence number", modifiedMessage[2], "received")
                    break
                else:
                    print("ACK mismatch. Resending packet", i)
            except socket.timeout:
                count += 1
                if count == 3:  # Maximum number of retries for acknowledgment
                    print("Timeout: Packet", i, "lost. Resending packet", i)
                    timeoutflag = 1
                    break
